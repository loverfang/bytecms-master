 ...
 */
@Component
public class CmsRecommDirective extends AbstractTemplateDirective {
    ...
    public void execute(RenderHandler handler) throws IOException, Exception {
        ...
        List<ContentDto> contentDtos=contentService.getTopContent(DirectiveNameEnum.CMS_RECOMM_DIRECTIVE,rowNum,categoryId);
        handler.listToMap(getDirectiveName().getCode(),contentDtos).render();
    }

    @PostConstruct
    public void initName(){
        this.setDirectiveName(DirectiveNameEnum.CMS_RECOMM_DIRECTIVE);
    }
}
